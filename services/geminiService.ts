...
export const generateMilestoneQuiz = async (title: string, description: string): Promise<QuizQuestion[]> => {
    const prompt = `Generate a 3-question quiz (JSON) for milestone: ${title}.
    Return format: { "questions": [ { "type": "MULTIPLE_CHOICE"|"TRUE_FALSE"|"SHORT_ANSWER", "question": "...", "options": ["..."], "correctAnswer": "..." } ] }`;
